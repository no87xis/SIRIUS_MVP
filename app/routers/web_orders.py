...
# MVP: Ручное создание заказов отключено - только через магазин
@router.get("/orders/new", response_class=HTMLResponse)
async def new_order_page_disabled(request: Request):
    """MVP: Заглушка для страницы создания заказа (функционал отключен)"""
    return RedirectResponse(url="/shop?info=Создание заказов только через магазин в MVP", status_code=302)

@router.post("/orders", response_class=HTMLResponse)
async def create_order_post_disabled(request: Request):
    """MVP: Заглушка для создания заказа (функционал отключен)"""
    return RedirectResponse(url="/shop?info=Создание заказов только через магазин в MVP", status_code=302)
