...
def get_order_statistics(db: Session) -> dict:
    ...
    courier_not_paid_count = db.query(func.count(Order.id)).filter(Order.status == OrderStatus.COURIER_NOT_PAID).scalar()
    ...
    return {
        ...
        "unpaid_count": db.query(func.count(Order.id)).filter(Order.status == OrderStatus.UNPAID).scalar() or 0,
        "paid_not_issued_count": pending_count,
        "paid_issued_count": issued_count,
        "courier_not_paid_count": courier_not_paid_count,
