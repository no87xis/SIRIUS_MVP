...
class PerformanceMonitor:
    """Оптимизированный сервис мониторинга производительности с ограничениями"""
    
    def __init__(self):
        # Ограничиваем размеры буферов для экономии памяти
        max_requests = getattr(settings, 'monitoring_max_requests', 100)  # По умолчанию 100
        max_db_queries = getattr(settings, 'monitoring_max_db_queries', 50)  # По умолчанию 50
        max_errors = getattr(settings, 'monitoring_max_errors', 20)  # По умолчанию 20
        
        self.request_times = deque(maxlen=max_requests)
        self.error_counts = defaultdict(int)
        self.database_query_times = deque(maxlen=max_db_queries)
        self.start_time = datetime.now()
        
        # Ограничиваем количество типов ошибок
        self._max_error_types = max_errors
        self._error_types_count = 0
    ...
    def record_error(self, error_type: str, path: str, error_message: str):
        """Запись ошибки с ограничением количества типов"""
        try:
            # Ограничиваем количество типов ошибок
            if error_type not in self.error_counts and self._error_types_count >= self._max_error_types:
                # Если достигли лимита, используем общий тип
                error_type = "OTHER_ERRORS"
            
            if error_type not in self.error_counts:
                self._error_types_count += 1
    ...
    def cleanup_old_data(self, max_age_hours: int = 24):
        ...
        try:
            ...
            self.request_times = deque(
                ...
                maxlen=self.request_times.maxlen
            )
            
            # Очищаем старые запросы к БД
            self.database_query_times = deque(
                [query for query in self.database_query_times if query['timestamp'] > cutoff_time],
                maxlen=self.database_query_times.maxlen
            )
            
            logger.debug(f"Очищены данные старше {max_age_hours} часов")
        except Exception as e:
